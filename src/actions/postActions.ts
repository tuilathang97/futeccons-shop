...
function pathConverter(str: string) {
  const accentsMap = {
    'à': 'a', 'á': 'a', 'ạ': 'a', 'ả': 'a', 'ã': 'a',
  ...
  return str
    .toLowerCase()
    .split('')
    .map((char) => accentsMap[char as keyof typeof accentsMap] || char) 
    .join('') 
    .replace(/[^a-z0-9]/g, '-') 
    .replace(/-+/g, '-') 
    .replace(/^-|-$/g, '');
...
export async function getPostByPath(path: string){
  const data = await db.query.postsTable.findFirst({
    // path la unique va khong trung nen dung find first
...
function preparePostData(formDataEntries: Record<string, string>, userId: string, isAdmin: boolean): Omit<typeof postsTable.$inferInsert, 'id' | 'createdAt' | 'updatedAt'> {
  ...
  return {
    ...
    path: pathConverter(formDataEntries.path || ''),
